class PokerBot:
    ...
    def learn(self, outcome):
        ...
        for i in range(len(self.history) - 1, -1, -1):
            state_key, action = self.history[i]
            if state_key not in self.strategy:
                self.strategy[state_key] = {}
            if action not in self.strategy[state_key]:
                self.strategy[state_key][action] = 0.0
            
            future_reward = 0.0
            if i < len(self.history) - 1:
                next_state_key, next_action = self.history[i + 1]
                future_reward = max(self.strategy[next_state_key].values())
            
            current_value = self.strategy[state_key][action]
            updated_value = current_value + self.learning_rate * (reward + self.discount_factor * future_reward - current_value)
            self.strategy[state_key][action] = updated_value
